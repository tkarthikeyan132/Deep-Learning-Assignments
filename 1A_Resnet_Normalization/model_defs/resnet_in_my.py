...
class InstanceNorm_My(nn.Module):
    def __init__(self, num_features, eps=1e-5, affine = True, device = "cuda"):
        super(InstanceNorm_My, self).__init__()
    ...
    def forward(self, x):
        ...
        mean = torch.mean(x, dim=(2, 3), keepdim=True) #num_features
        var = torch.var(x, dim=(2, 3), unbiased=False, keepdim=True) #num_features
        ...
        # Scale and shift
        scaled_x = normalized_x * self.gamma.reshape(1,-1,1,1) + self.beta.reshape(1,-1,1,1) #N x C x H x W
