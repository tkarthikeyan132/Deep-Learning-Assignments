...
#Arch 3: BERT + Attn LSTM Decoder
# from model_bert_attn import *

#Arch 4: BERT adapter + Attn LSTM Decoder
# from model_bert_attn_adapter import *
from model_bert_attn_unfreeze import *
...
train_dataset = ProblemFormulaDatasetBert(train_data, formula_vocab, bert_tokenizer)
train_dataloader = DataLoader(train_dataset, batch_size=BATCH_SIZE, collate_fn=collate_fn_bert)

val_dataset = ProblemFormulaDatasetBert(val_data, formula_vocab, bert_tokenizer)
val_dataloader = DataLoader(val_dataset, batch_size=BATCH_SIZE, collate_fn=collate_fn_bert)
...
# for problem, attn_mask, linear_formula in train_dataloader:
#     print(problem.shape)
...
def train():
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for problem, attn_masks, linear_formula in tqdm(train_dataloader):
            input_tensor, attn_tensor, target_tensor = problem.to(device), attn_masks.to(device), linear_formula.to(device)
        ...
        with torch.no_grad():
            total_val_loss = 0
            for val_problem, val_attn_masks, val_linear_formula in tqdm(val_dataloader):
                val_input_tensor, val_attn_tensor, val_target_tensor = val_problem.to(device), val_attn_masks.to(device), val_linear_formula.to(device)

                val_encoder_outputs, val_encoder_hidden, val_encoder_cell = encoder(val_input_tensor, val_attn_tensor)
    ...
    store_json_file(train_loss_list, val_loss_list, HIDDEN_DIM, BATCH_SIZE, EMBED_DIM, LEARNING_RATE, EPOCHS, output_dir)
...
def test(file_path, store_file_path, instance):
    ...
    for data in tqdm(test_data):
        ...
        input_tensor, attn_tensor = train_dataset.preprocess_problem(question)
...
def main():    
    ...
    instance = "final"
    # instance = 6
    test(file_path=file_path + "dev.json", store_file_path=output_dir + f"dev_predicted_{instance}_bm20.json", instance=instance)

    test(file_path=file_path + "test.json", store_file_path=output_dir + f"test_predicted_{instance}_bm20.json", instance=instance)

    print("Dev data:")
    os.system("python data/evaluator.py " + output_dir + f"dev_predicted_{instance}_bm20.json")

    print("Test data:")
    os.system("python data/evaluator.py " + output_dir + f"test_predicted_{instance}_bm20.json")
