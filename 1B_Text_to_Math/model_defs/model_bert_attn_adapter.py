...
class EncoderRNN(nn.Module):
    def __init__(self):
        ...
        self.linear2 = nn.Linear(in_features=768, out_features=128)
        self.linear3 = nn.Linear(in_features=768, out_features=128)

    def forward(self, input_ids, attention_mask):
        outputs = self.bert(input_ids=input_ids, attention_mask=attention_mask)
        hidden_states = self.linear1(outputs.last_hidden_state)
        hidden_state = self.linear2(outputs.last_hidden_state[:,0,:].unsqueeze(dim=0))
        cell_state = self.linear3(outputs.pooler_output.unsqueeze(dim=0))
